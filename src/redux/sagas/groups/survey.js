...
// save answers
function* performSaveAnswers(action) {
  try {
    const result = yield call(saveSurveyAnswers, action.data);
    if (result?.data?.success) {
      yield put(saveAnswersComplete({ data: result.data }));
    } else yield put(saveAnswersComplete({ data: result.data }));
  } catch (e) {
    console.log({ error: e });
